...
ASTFieldVariable *ASTResource::addASTFieldVariable(VarDecl *VD, FieldDecl *FD, ASTFunction *F) {
  unsigned id = F->getFieldVariables().size();
  ASTFieldVariable *V = new ASTFieldVariable(id, VD, FD, F);
  ASTFieldVariables.push_back(V);
  F->addFieldVariable(V);
  return V;
}
...
void ASTBimap::insertFieldVariable(ASTFieldVariable *V) {
  fieldVariableSet.emplace(V);
}

void ASTBimap::insertVar2Field(ASTVariable* V, std::set<ASTFieldVariable *> F) {
  var2fields.emplace(V, F);
}
...
ASTFieldVariable *ASTBimap::getASTFieldVariable(VarDecl *VD, FieldDecl *FD, std::vector<FieldDecl *> fathers) {
  for(auto it = fieldVariableSet.begin(); it != fieldVariableSet.end(); ++it) {
    ASTFieldVariable *current = *it;
    if(current->getVar() == VD && current->getFieldDecl() == FD) {
      std::vector<FieldDecl *> current_fathers;
      while(current->hasFather()) {
        current_fathers.push_back(current->getFather()->getFieldDecl());
        current = current->getFather();
      }
      if(fathers == current_fathers) return *it;
    }
  }
  return nullptr;
}

std::set<ASTFieldVariable *> ASTBimap::getAllFields(ASTVariable *V) {
  auto it = var2fields.find(V);
  if(it == var2fields.end()) {
    std::set<ASTFieldVariable *> empty_set;
    return empty_set;
  }
  return it->second;
}
...
ASTManager::ASTManager(std::vector<std::string> &ASTs, ASTResource &resource,
  ...
  for (std::string AST : ASTs) {
    ...
    for (FunctionDecl *FD : functions) {
      ...
      //classFields不加入到ASTResource或ASTBimap中
      //添加class所有成员变量
      ...
      for (VarDecl *VD : variables) {
        ASTVariable *root = resource.addASTVariable(VD, F);
        if(isRecord(VD)) {
          addAllFields(root, VD, resource, F);
        }
      }
...
void ASTManager::addAllFields(ASTVariable *root, VarDecl *VD, ASTResource &resource, ASTFunction *F) {
  RecordDecl *rd = getRecord(VD);

  std::queue<ASTFieldVariable *> nestingFields;

  if(!rd) return;
  for(RecordDecl::field_iterator iter = rd->field_begin(); iter != rd->field_end(); ++iter) {
    FieldDecl *fd = *iter;
    ASTFieldVariable *current = resource.addASTFieldVariable(VD, fd, F);
    root->addField(current);
    if(isRecord(fd)){ 
      nestingFields.push(current);
    }
  }

  while(!nestingFields.empty()) {
    ASTFieldVariable *father = nestingFields.front();
    nestingFields.pop();
    RecordDecl *father_rd = getRecord(father->getFieldDecl());

    if(!father_rd) continue;
    for(RecordDecl::field_iterator iter = father_rd->field_begin(); iter != father_rd->field_end(); ++iter) {
      FieldDecl *fd_nest = *iter;
      ASTFieldVariable *child_field = resource.addASTFieldVariable(VD, fd_nest, F);
      child_field->setFather(father);
      father->addNestingField(child_field);

      if(isRecord(fd_nest) && !child_field->isSameFieldAsFather()){
        nestingFields.push(child_field);
      } 
    }
  }
}

void ASTManager::addAllClassFields(ASTFunction *F, FunctionDecl *FD) {
  int ID = 0;
  std::queue<ASTFieldVariable *> fields;

  if(CXXMethodDecl *MD = dyn_cast<CXXMethodDecl>(FD)) {
    CXXRecordDecl *parent = MD->getParent();
    for(auto iter = parent->field_begin(); iter != parent->field_end(); ++iter) {
      FieldDecl *fd = *iter;
      ASTFieldVariable *field_new = new ASTFieldVariable(ID, fd, F);
      field_identity iden;
      iden.field = fd;
      F->addClassField(field_new);
      ++ID;
      if(isRecord(fd)) {
        fields.push(field_new);
      }
    }

    while(!fields.empty()) {
      ASTFieldVariable *father = fields.front();
      fields.pop();
      RecordDecl *rd = getRecord(father->getFieldDecl());
      assert(rd);
      for(auto it = rd->field_begin(); it != rd->field_end(); ++it) {
        FieldDecl *fd_nest = *it;
        //fd_nest->dump();
        ASTFieldVariable *fv_child = new ASTFieldVariable(ID, fd_nest, F);
        ++ID;  
        fv_child->setFather(father);
        father->addNestingField(fv_child);
        F->addClassField(fv_child);
        if(isRecord(fd_nest) && !fv_child->isSameFieldAsFather()) 
          fields.push(fv_child);
      }
    }
  }
...
std::set<ASTFieldVariable *> ASTManager::getAllFields(ASTVariable *V) {
  return bimap.getAllFields(V);
}

ASTFieldVariable *ASTManager::getASTFieldVariable(VarDecl *VD, FieldDecl *FD, std::vector<FieldDecl *> fathers) {
  return bimap.getASTFieldVariable(VD, FD, fathers);
}
...
void ASTManager::push(std::unique_ptr<ASTUnit> AU) {
  ...
  for (unsigned i = 0; i < functions.size(); i++, ASTFuncIndex++) {
    ...
    const std::vector<ASTFieldVariable *> &ASTFieldVariables = F->getFieldVariables();
    for(ASTFieldVariable *v : ASTFieldVariables) {
      //v->getFieldDecl()->dump();
...
bool ASTManager::isRecord(VarDecl *vd) {
...
bool ASTManager::isRecord(FieldDecl *fd) {
...
RecordDecl *ASTManager::getRecord(VarDecl *VD) {
  if(VD->getType()->isRecordType()) {
...
RecordDecl *ASTManager::getRecord(FieldDecl *FD) {
  if(FD->getType()->isRecordType()) {
...
clang::QualType ASTManager::getRealType(VarDecl *VD) {
  clang::QualType type = VD->getType();
  while(type->isPointerType()) {
    type = type->getPointeeType();
  }
...
clang::QualType ASTManager::getRealType(FieldDecl *FD) {
  clang::QualType type = FD->getType();
  while(type->isPointerType()) {
    type = type->getPointeeType();
  }
