...
/**
 * get all field_variables (e.g. a.f)
 */
std::vector<std::pair<VarDecl *, FieldDecl *>> getFieldVariables(FunctionDecl *FD) {
  std::vector<std::pair<VarDecl *, FieldDecl *>> field_variables;
  
  ASTFieldVariableLoad load;
  load.TraverseStmt(FD->getBody());
  field_variables.insert(field_variables.end(), load.getFieldVariables().begin(),
                   load.getFieldVariables().end());
  //if(field_variables.size() == 0) llvm::errs() << "field variables unfound\n";
  return field_variables;
}
...
std::vector<std::tuple<VarDecl *, FieldDecl *, FieldDecl *>> getNestingFields(FunctionDecl *FD) {
  ...
  nesting_fields.insert(nesting_fields.end(), load.getNestingFields().begin(),
                    load.getNestingFields().end());
  return nesting_fields;
}
